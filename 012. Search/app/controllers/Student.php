  ...
  class Student extends Controller {
    ...
    public function delete($id) {
      ...
      if($this->model("StudentModel")->delete($id) > 0) {
        Flasher::setFlash("sucesssfully", "deleted", "success");
      } else {
        Flasher::setFlash("unsuccessfully", "deleted", "danger");
      }
    }

    // Searches for students based on the given search term
    public function search() {
      $data["title"]     = "Students";
      $data["students"]  = $this->model("StudentModel")->search($_POST["search"]);
